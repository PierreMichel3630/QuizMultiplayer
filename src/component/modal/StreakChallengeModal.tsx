...
export const StreakChallengeModal = ({ streak, close }: Props) => {
  ...
  useEffect(() => {
    const streakValue =
      streak > MAX_DAY_RECOMPENSES_STREAK ? MAX_DAY_RECOMPENSES_STREAK : streak;
    const recompense = RECOMPENSES_STREAK.find((el) => el.day === streakValue);
    if (recompense) {
      setRecompense(recompense);
    }
  }, [streak]);
  return (
    <Dialog onClose={close} open={true} maxWidth="sm">
      <DialogContent sx={{ p: 2, pt: 3 }}>
        <Grid container spacing={2} justifyContent="center">
          <Grid item xs={12}>
            <Box sx={{ display: "flex", justifyContent: "center" }}>
              <StreakBlock value={streak ?? 0} logoSize={40} textSize={35} />
            </Box>
          </Grid>
