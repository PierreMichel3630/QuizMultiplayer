...
import { encrypt } from "../_shared/response.ts";
    ...
    if (newQuestion) {
      ...
      if (qcm) {
        if (newQuestion.typequestion === "ORDER") {
          ...
          const responseIndex = [...responsesQcm].findIndex(
            (el) => el.label === responseOrder.name
          );
          response = responseIndex;
          newQuestion = { ...newQuestion, response: responseIndex };
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (value === null) {
      ...
      const key = Deno.env.get("CRYPT_SECRET");
      return new Response(
        JSON.stringify({
          ...
          data: question.data,
          response: encrypt(JSON.stringify(response), key),
        }),
        {
          headers: { ...corsHeaders, "Content-Type": "application/json" },
          status: 200,
