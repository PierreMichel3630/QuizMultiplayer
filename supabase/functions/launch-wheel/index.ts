  ...
  if (req.method === "OPTIONS") {
    ...
    try {
      ...
      if(timeHours>= 24) {
        ...
        newmoney = option.type === TypeWheelEnum.GOLD ? previousmoney + value : previousmoney
        newxp = option.type === TypeWheelEnum.XP ? previousxp + value : previousxp

        await supabase.from("profiles").update({money : newmoney, wheellaunch: new Date()}).eq("id", user.id)

        await supabase.from("stataccomplishment").update({xp : newxp}).eq("profile", user.id)
      }
